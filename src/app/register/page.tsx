...
export default function RegisterPage() {
  ...
  const captureFace = async () => {
    if (!faceapi) {
      setStatus("Face API not loaded yet")
      return
    }
    ...
    <>
      ...
      <main className="min-h-screen flex flex-col items-center justify-center bg-gray-100 gap-4">
        ...
        <form onSubmit={handleRegister} className="flex flex-col gap-4 w-80">
          <input type="text" placeholder="Full Name" value={name} onChange={(e) => setName(e.target.value)} required className="px-4 py-2 border rounded"/>
          <input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} required className="px-4 py-2 border rounded"/>
          <input type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} required className="px-4 py-2 border rounded"/>

          <div className="flex gap-2">
            <select value={selectedLocation} onChange={(e) => setSelectedLocation(e.target.value)} required className="flex-1 px-4 py-2 border rounded">
              <option value="">Select your location</option>
              {locations.map((loc) => (
                <option key={loc.id} value={loc.id}>{loc.name}</option>
              ))}
            </select>
            <button type="button" className="px-3 py-2 bg-gray-300 rounded hover:bg-gray-400" onClick={() => setShowAddLocation(true)}>+</button>
          </div>
          ...
          {showAddLocation && (
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                ...
                <button type="button" className="px-3 py-2 bg-red-500 text-white rounded hover:bg-red-600" onClick={() => setShowAddLocation(false)}>Cancel</button>
              </div>
            </div>
          )}

          <button type="submit" className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition">Register</button>
        </form>

        <p className="mt-2 text-dark">or capture your face below</p>
        <button onClick={captureFace} className="px-4 py-2 bg-green-500 text-dark rounded hover:bg-green-600 transition">Capture Face</button>
